...
- **Menú de perfiles de batería (waybar → click batería)** — 2026-06-03: el click hace `omarchy-menu power` → `show_setup_power_menu`, que lista perfiles con el comando `powerprofilesctl`. En Asahi/Fedora ese binario **no existe** (gestor = `tuned-ppd`, no `power-profiles-daemon`), así que el menú salía **vacío**. Pero `tuned-ppd` **sí expone la interfaz D-Bus estándar** `net.hadess.PowerProfiles` con 3 perfiles (`power-saver`/`balanced`/`performance`). Fix: shim `~/.local/bin/powerprofilesctl` (versionado en `bin/powerprofilesctl`) que implementa `list`/`get`/`set` vía `gdbus` sobre ese D-Bus — el `set` no pide polkit y tuned mapea solo (`performance`→`throughput-performance`, etc.). NO instalar `power-profiles-daemon` (entra en conflicto con `tuned-ppd`). `~/.local/bin` va antes que `/usr/bin` en el PATH (incl. el de waybar), así que el shim gana.
...
- **Selector en el menú**: **Super+Alt+Space → Setup → Monitor Mode** → script **`~/.local/bin/omarchy-monitor-mode`** (usa el mismo walker `--dmenu` de Omarchy). Modos: **Clonar (espejo)**, **Extender derecha/izquierda** (escala 1.5, `auto-right`/`auto-left`), **Solo externo**, **Solo laptop**. Detecta el externo dinámicamente (no hardcodea el nombre), aplica y **persiste** regenerando `monitors.conf` + `hyprctl reload`.
  ...
  - **waybar no aparecía en el externo al extender**: al pasar de espejo→extender, Hyprland NO re-anuncia el `wl_output` del externo y waybar no lo detecta (sí funciona al desconectar/reconectar el cable). Fix: el script **simula el re-enchufe** (`hyprctl keyword monitor "$EXT, disable"` + `hyprctl reload`) → recrea el output → el guardián de hotplug (abajo) refresca waybar al ver el `monitoradded`. Sin reinicio manual de waybar.
- **Guardián de hotplug** `~/.local/bin/monitor-hotplug-guard` (`exec-once` en `autostart.conf`): escucha el socket2 de Hyprland. (1) Al **desconectar** un monitor, si el interno quedó sin activar (venías en "Solo externo") **lo reactiva** → evita pantalla negra sin retorno (bug del "Solo externo"). (2) Refresca **waybar** al conectar/desconectar para que dibuje las barras en los outputs actuales. Verificado: deshabilitar eDP-1 lo reactiva solo; extender pone waybar en ambos.
  - ⚠️ "Solo externo" deja `eDP-1, disable`: si desconectas el externo, el guardián reactiva el interno; aun así puedes elegir "Solo laptop".
