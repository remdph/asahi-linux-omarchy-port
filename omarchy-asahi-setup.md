...
- Env para el backend: `autostart.conf` exporta `WAYLAND_DISPLAY XDG_CURRENT_DESKTOP HYPRLAND_INSTANCE_SIGNATURE` vía `dbus-update-activation-environment`.
- Verificar: `busctl --user introspect org.freedesktop.portal.Desktop /org/freedesktop/portal/desktop | grep ScreenCast`. Backends: `xdg-desktop-portal-hyprland` + `-gtk` activos; KDE enmascarado.
...
- **Sesión se cierra al cerrar una ventana (crash hyprscroller)** — **DOS bugs distintos**, ambos en la ruta de quitar ventana (SIGSEGV de Hyprland → vuelve al login). Fuente parcheada en `~/.local/src/hyprscroller`, recompilada con `cd ~/.local/src/hyprscroller && make release` e instalada en `~/.local/share/hyprland/hyprscroller.so`. **Activo solo desde el siguiente login** (no hot-reload: descargar el plugin puede crashear). Reconstruir: si la caché de CMake apunta a otro dir, `rm -rf Release` antes de `make release`.
  1. **`Column::remove_window` null-deref** (2026-05-30): cherry-pick del commit upstream `248d5c1` sobre el tag 0.51.1. Backup `.bak-buggy-20260530`.
  2. **`Row::~Row()` use-after-free** en `ScrollerLayout::onWindowRemovedTiling` (al cerrar la **última** ventana de un row) (2026-05-30): el código hacía `rows.erase(row); delete row->data();` pero `list.h::erase` hace `delete it` (libera el ListNode) → `row->data()` después es UAF → destructor sobre puntero basura. Fix en `src/scroller.cpp`: usar `delete s;` (el puntero al Row ya conocido) en vez de `row->data()`. Backup previo `.bak-colfix-20260530`.
