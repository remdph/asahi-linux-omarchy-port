...
- **Enchufado a AC: todos los tiempos ×5** (pantalla 10 min, lock 10 min, suspender **50 min**). hypridle no tiene perfiles AC/batería, así que `hypridle.conf` se **genera** y hypridle se reinicia al cambiar de corriente:
  ...
  - **No suspender mientras corre Claude Code** — 2026-06-03: el listener de *suspender* lleva un guard `on-timeout = pgrep -x claude >/dev/null || systemctl suspend`. Si hay un proceso `claude` vivo, el `||` corta el comando y el equipo **no se suspende** (las tareas en marcha no se cortan). Atenuar/apagar pantalla/lock siguen igual. **Sin polling**: aprovecha el temporizador propio de hypridle (reevalúa el `pgrep` en cada ciclo de idle). El guard está en `hypridle-apply` (no en el conf, que se regenera). Copia versionada del script en `bin/hypridle-apply`. Para extenderlo a apagar pantalla/lock, anteponer el mismo `pgrep -x claude >/dev/null ||` a esos `on-timeout`.
